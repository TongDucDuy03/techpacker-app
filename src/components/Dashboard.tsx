...
export const Dashboard: React.FC<DashboardProps> = ({ techPacks, activities = [] }) => {
  ...
  const stats = [
    ...
    {
      title: t('dash.stat.pending'),
      value: techPacks.filter(tp => tp.status === 'pending_approval').length.toString(),
      change: '-3%',
      trend: 'down',
      icon: Clock,
      color: 'bg-yellow-500'
    ...
    {
        title: t('dash.stat.draft'),
        value: techPacks.filter(tp => tp.status === 'draft').length.toString(),
        change: '+8%',
        trend: 'up',
        icon: Package,
        color: 'bg-green-500'
          ...
          <div className="p-6">
            <div className="space-y-4">
              {techPacks.slice(0, 5).map((techPack) => (
                <div key={techPack._id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center space-x-3">
                    <img
                      src={techPack.metadata?.images?.[0]?.thumbnail || 'https://via.placeholder.com/150'}
                      alt={techPack.name}
                      className="w-12 h-12 rounded-lg object-cover"
                    />
                    <div>
                      <h4 className="font-medium text-gray-900">{techPack.name}</h4>
                      <p className="text-sm text-gray-500">{techPack.articleCode}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      techPack.status === 'draft' ? 'bg-blue-100 text-blue-800' :
